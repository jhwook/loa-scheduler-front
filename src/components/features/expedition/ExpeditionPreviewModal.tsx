...
export function ExpeditionPreviewModal({ open, onClose }: Props) {
  ...
  return createPortal(
    <>
        ...
        >
          <div className="border-b border-base-300 px-5 py-4">
            ...
            >
              ...
              <button
                type="submit"
                disabled={loading}
                className="btn btn-primary shrink-0 rounded-xl text-primary-content sm:mb-0"
              >
                {loading ? '검색 중…' : '검색'}
              </button>
            </form>
                          ...
                          >
                            ...
                            <div className="min-w-0 flex-1">
                              <div className="flex min-w-0 items-center gap-2">
                                <p className="min-w-0 truncate text-sm font-semibold text-base-content">
                                  {c.characterName}
                                </p>
                              </div>
                              <p className="mt-1 text-xs text-base-content/60">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 border-t border-base-300 bg-base-200/90 px-5 py-3">
            <p className="text-xs text-base-content/60">
              선택 {selected.size} / {rows.length}명 · 체크한 캐릭터만
              저장됩니다.
            </p>
            <button
              type="button"
              disabled={saving || rows.length === 0}
              className="btn btn-primary btn-sm rounded-lg px-5 font-semibold text-primary-content disabled:opacity-50"
      ...
      {confirmSaveOpen ? (
        <div
          className="fixed inset-0 z-120 flex min-h-dvh items-center justify-center bg-base-300/70 p-4"
          role="alertdialog"
          aria-modal="true"
          aria-labelledby="expedition-save-confirm-title"
        >
          <div className="w-full max-w-sm rounded-2xl border border-base-300 bg-base-200 p-6 shadow-2xl">
            <p
              id="expedition-save-confirm-title"
              className="text-center text-base font-semibold text-base-content"
            >
              저장하시겠습니까?
            </p>
            ...
            <div className="mt-6 flex justify-center gap-2">
              <button
                type="button"
                className="btn btn-ghost flex-1 border border-base-300 font-semibold text-base-content"
                disabled={saving}
              ...
              <button
                type="button"
                className="btn btn-primary flex-1 font-semibold text-primary-content"
                disabled={saving}
      ...
      {saveFeedback ? (
        <div
          className="fixed inset-0 z-130 flex min-h-dvh items-center justify-center bg-base-300/70 p-4"
          role="alertdialog"
          aria-modal="true"
        >
          <div
            className={`w-full max-w-sm rounded-2xl border bg-base-200 p-6 shadow-2xl ${
          ...
          >
            ...
            <div className="mt-6 flex justify-end">
              <button
                type="button"
                className="btn btn-primary px-8 font-semibold text-primary-content"
                onClick={() => {
