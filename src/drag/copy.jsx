...
//       // 왼쪽에서 오른쪽으로 드래그
//       const sourceItems = Array.from(leftItems);
//       const [movedItem] = sourceItems.splice(source.index, 1);

//       const boxIndex = rightBoxes.findIndex(
//         (box) => box.id === destination.droppableId
//       );

//       if (boxIndex !== -1 && rightBoxes[boxIndex].items.length < 8) {
//         const updatedBoxes = [...rightBoxes];
//         updatedBoxes[boxIndex].items.push(movedItem);

//         setLeftItems(sourceItems);
//         setRightBoxes(updatedBoxes);
//       } else {
//         alert('이 틀에는 최대 8개의 아이템만 추가할 수 있습니다.');
//       }
//     } else if (source.droppableId.startsWith('box')) {
//       // 오른쪽 박스 내에서 이동 또는 왼쪽으로 복귀
//       const sourceBoxIndex = rightBoxes.findIndex(
//         (box) => box.id === source.droppableId
//       );
//       const sourceItems = Array.from(rightBoxes[sourceBoxIndex].items);
//       const [movedItem] = sourceItems.splice(source.index, 1);

//       if (destination.droppableId === 'left') {
//         setLeftItems([...leftItems, movedItem]);
//         const updatedBoxes = [...rightBoxes];
//         updatedBoxes[sourceBoxIndex].items = sourceItems;
...
//             style={styles.addInput}
//           />
//           <button onClick={handleSaveTitle} style={styles.addButton}>
//             Save Title
//           </button>
//           <button
